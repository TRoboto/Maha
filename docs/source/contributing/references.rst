...
type, and in general ``:class:`<path>`â€‹`` to refer to a certain class
...
Example: ``:class:`int`â€‹``, ``:class:`str`â€‹``, ``:class:`float`â€‹``,
``:class:`bool`â€‹``
...
Example: ``:class:`MyClass`â€‹`` referring to a class in the same file;
``:meth:`push`â€‹`` referring to a method in the same class;
``:meth:`MyClass.push`â€‹`` referring to a method in a different class in
the same file; ``:attr:`color`â€‹`` referring to an attribute in the same
class; ``:attr:`MyClass.color`â€‹`` referring to an attribute in a
different class in the same file.

3. If it's on a different file, then you may either use the full dotted
   name (e.g. ``~maha.processors.TextProcessor``) or simply use the
   shortened way (``~.TextProcessor``). Note that, if there is ambiguity,
   ...
   displays just ``TextProcessor`` instead of the full location in the
   rendering. It can be removed for disambiguation purposes only.

Example: ``:class:`~.TextProcessor`â€‹``, ``:meth:`~.TextProcessor.keep`â€‹``

1. If it's a class from a different module, specify the full dotted
   syntax.

Example: ``:class:`numpy.ndarray`â€‹`` for a numpy ndarray.
...
1. Class names should be wrapped in ``:class:`path_goes_here`â€‹``. See
   examples in the subsection above.
2. Method names should be wrapped in ``:meth:`path_goes_here`â€‹``. See
   examples in the subsection above.
3. Attribute names should be wrapped in ``:attr:`path_goes_here`â€‹``. See
   examples in the subsection above.
...
Example: ``Dict[:class:`str`, :class:`~.Expression`]`` is a dictionary that
maps strings to Expressions.
...
7. **If the parameter is a list:** Note that it is very rare to require
   ...
   ``Iterable[:class:`~.Expression`]`` for an iterable of Expressions; etc.
